...
//4
//si on souhaite supprimer un examen  il faut supprimer les questions et réponses associées car contraintes de clés
...
app.delete('/examen/:id/:id_examen', async (req, res) => {
    ...
    try {
        ...
        //supprimer l'examen de repond
        await client.query('DELETE FROM repond WHERE id_question IN (SELECT id from question where id_examen =$1)', [id_examen]);

        //supprimer de invitation
        await client.query('DELETE FROM invitation WHERE id_examen =$1', [id_examen]);
...
app.put("/lancer_examen/:id/:id_examen", async (req,res) => {
    try{
        const {id,id_examen} = req.params;
        const updateSistuation = await pool.query("UPDATE examen SET situation= 'active' WHERE id=$1",[id_examen]);
...
app.put("/correction_auto/:id/:id_examen", async (req, res) => {
    try {
        ...
        console.log("je suis avant le requette quin permts de recuperer les id des questions");
        ...
        for (let i = 0; i < arrayIdQuestion.length; i++) {
            console.log("je suis la avnat la requette pour calculer le nombre de poitn obetnu par la question numero",i+1);
            ...
            switch (falseAnswers) {
                case 0:  //si tous le reponse sont corrects
                    ...
                    console.log(arrayIdQuestion[i].nb_points);
                    break;
...
//retourne les notes des examens passés en paramètres
...
app.post("/Envoyer_reponses_examine/:id/:id_examen", async (req, res) => {
    ...
    try {
        ...
        await client.query('COMMIT');
        console.log("je suis avant la correction auto");
        const url = `http://localhost:3000/correction_auto/${id}/${id_examen}`;
        const response = await fetch(url, { method: 'PUT' });
        const correctionResult = await response.json();
        console.log("je suis apres la correction auto");
    ...
    try {
        ...
        for (let i = 0; i < arrayIdQuestion.length; i++) {
            const aux = await pool.query(`
                ...
                [id, arrayIdQuestion[i].id]);
            const falseAnswers = parseInt(aux.rows[0].count, 10);
            bareme_totale += arrayIdQuestion[i].nb_points;
            switch (falseAnswers) {
                case 0:  //si tous le reponse sont corrects
                    notes += arrayIdQuestion[i].nb_points;
                    break;
        ...
        await pool.query("UPDATE invitation SET note = $1 WHERE id_examine = $2", [notes, id]);
        res.json({ message: 'La note est saisie dans la BDD avec succes', note: notes });
        await pool.query("UPDATE examen SET statut = 'corrige' WHERE id = $1",[id_examen]);
    } catch (error) {
        console.error('Error during automatic correction:', error);
        res.status(500).send('Internal Server Error');
}});
